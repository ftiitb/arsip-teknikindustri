...
function reportSelect(bt){
	...
	options = grid_pejabat.getParamsFilter();
...
var grid_pejabat = new Ext.ux.PhpDynamicGridPanel({
    border:false,
    remoteSort:true, //optional default true
    autoLoadStore:true, //optional default true
    storeUrl:ajax_url,
    sortInfo:{field:'jabatan',direction:'ASC'}, //must declaration
    baseParams:{
      action:'read'
    },
    tbar:[
    ...
    onAddData:function(bt){
      win_pejabat.get(1).getForm().reset();
      win_pejabat.setTitle('Add Data'); 
      win_pejabat.show(bt.id); 
    },
    onEditData:function(bt,rec){
      win_pejabat.setTitle('Edit Data');
      win_pejabat.show(bt.id); 
      win_pejabat.get(1).getForm().load({
...
win_pejabat = new Ext.Window({
  ...
  items:[{
	...
	html: '<div id="winheader"><label>DATA PEJABAT</label></div>',
	height:60
  },
  {
    xtype:'form',
    ...
    {xtype:'textfield',name:'jabatan',fieldLabel:'Jabatan',allowBlank:false,maxLength: 255,maxLengthText: 'Maximum 255 karakter.'},
	{xtype:'textfield',name:'namapejabat',fieldLabel:'Nama Pejabat',allowBlank:false,maxLength: 255,maxLengthText: 'Maximum 255 karakter.'},
    {xtype:'textfield',name:'nippejabat',fieldLabel:'NIP',allowBlank:false,maxLength: 50,maxLengthText: 'Maximum 50 karakter.'}
    ]
  }], 
  buttons:[
  {
	...
	iconCls:'icon-save',
    handler:function(){
      if(!win_pejabat.get(1).getForm().isValid()){
        Ext.example.msg('Peringatan','Ada data yang kosong'); 
        return false; 
      }
      
      id_data = win_pejabat.get(1).getForm().getValues().id; 
      action = (id_data?'update':'create'); 
      win_pejabat.get(1).getForm().submit({
          params:{action:action},
          waitMsg : 'Saving Data',
          success:function(){
            win_pejabat.hide();
            Ext.example.msg('Simpan','Data telah disimpan'); 
            grid_pejabat.store.reload(); 
          },
          failure:function(){
