...
function reportSelect(bt){
	...
	options = grid_individu.getParamsFilter();
...
var grid_individu = new Ext.ux.PhpDynamicGridPanel({
    border:false,
    ...
    onAddData:function(bt){
      win_individu.get(1).getForm().reset();
      win_individu.setTitle('Add Data'); 
      win_individu.show(bt.id); 
    },
    onEditData:function(bt,rec){
      win_individu.setTitle('Edit Data');
      win_individu.show(bt.id); 
      win_individu.get(1).getForm().load({
...
win_individu = new Ext.Window({
  ...
  items:[{
	...
	html: '<div id="winheader"><label>DATA INDIVIDU</label></div>',
	height:60
  },
  {
    xtype:'form',
    ...
    {xtype:'textfield',name:'nama',fieldLabel:'Nama',allowBlank:false,maxLength: 255,maxLengthText: 'Maximum 255 karakter.'},
	{xtype:'textfield',name:'email',fieldLabel:'Email',allowBlank:false,maxLength: 255,maxLengthText: 'Maximum 255 karakter.'},
    {xtype:'textfield',name:'status',fieldLabel:'Status',allowBlank:false,maxLength: 5,maxLengthText: 'Maximum 5 karakter.'}
    ]
  }], 
  buttons:[
  {
	...
	iconCls:'icon-save',
    handler:function(){
      if(!win_individu.get(1).getForm().isValid()){
        Ext.example.msg('Peringatan','Ada data yang kosong'); 
        return false; 
      }
      
      id_data = win_individu.get(1).getForm().getValues().id; 
      action = (id_data?'update':'create'); 
      win_individu.get(1).getForm().submit({
          params:{action:action},
          waitMsg : 'Saving Data',
          success:function(){
            win_individu.hide();
            Ext.example.msg('Simpan','Data telah disimpan'); 
            grid_individu.store.reload(); 
          },
          failure:function(){
...
var main_content = {
  ...
  listeners:{
    destroy:function(){
      my_win = Ext.getCmp('win-individu');
      if (my_win)
          my_win.destroy(); 
    }
  }
