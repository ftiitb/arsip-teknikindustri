...
function reportSelect(bt){
	...
	options = grid_kodearsip.getParamsFilter();
...
var grid_kodearsip = new Ext.ux.PhpDynamicGridPanel({
    border:false,
    ...
    onAddData:function(bt){
      win_kodearsip.get(1).getForm().reset();
      win_kodearsip.setTitle('Add Data'); 
      win_kodearsip.show(bt.id); 
    },
    onEditData:function(bt,rec){
      win_kodearsip.setTitle('Edit Data');
      win_kodearsip.show(bt.id); 
      win_kodearsip.get(1).getForm().load({
...
win_kodearsip = new Ext.Window({
  ...
  buttons:[
  {
	...
	iconCls:'icon-save',
    handler:function(){
      if(!win_kodearsip.get(1).getForm().isValid()){
        Ext.example.msg('Peringatan','Ada data yang kosong'); 
        return false; 
      }
      
      id_data = win_kodearsip.get(1).getForm().getValues().id; 
      action = (id_data?'update':'create'); 
      win_kodearsip.get(1).getForm().submit({
          params:{action:action},
          waitMsg : 'Saving Data',
          success:function(){
            win_kodearsip.hide();
            Ext.example.msg('Simpan','Data telah disimpan'); 
            grid_kodearsip.store.reload(); 
          },
          failure:function(){
...
var main_content = {
  ...
  items : [grid_kodearsip],
  listeners:{
    destroy:function(){
      my_win = Ext.getCmp('win-kodearsip');
      if (my_win)
          my_win.destroy(); 
    }
  }
