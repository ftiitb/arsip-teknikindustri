<?php
  ...
  $objPHPExcel->setActiveSheetIndex(0)
              ->setCellValue('A1', 'JABATAN')
			  ->setCellValue('B1', 'NAMA PEJABAT')
              ->setCellValue('C1', 'NIP'); 

  $index = 2;             
  while ($row=$rs->FetchNextObject()) {
    $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('A'.$index, $row->JABATAN)
				->setCellValue('B'.$index, $row->NAMAPEJABAT)
                ->setCellValue('C'.$index, $row->NIPPEJABAT);  
    $index++;                              
  }
  ...
  header('Content-Disposition: attachment;filename="PEJABAT.xlsx"');
