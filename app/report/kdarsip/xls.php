<?php
  ...
  $objPHPExcel->setActiveSheetIndex(0)
              ->setCellValue('A1', 'KODE ARSIP')
              ->setCellValue('B1', 'DESKRIPSI'); 

  $index = 2;             
  while ($row=$rs->FetchNextObject()) {
    $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('A'.$index, $row->KODEARSIP)
                ->setCellValue('B'.$index, $row->DESKRIPSI);  
    $index++;                              
  }
  ...
  header('Content-Disposition: attachment;filename="KODEARSIP.xlsx"');
